...
Backend Alignment V4 updates the Android client to match the current mobile contract:
...
- Booking and reschedule pickers use the configured 09:00-17:00 clinic window with 15-minute increments; Sunday remains unavailable. Android submits picker values as ISO-8601 timestamps with the explicit `Asia/Manila` offset (`+08:00`) and converts offset-bearing API responses to `Asia/Manila` before display and date grouping.
...
See `docs/specs/backend-alignment-v4-spec.md` for implementation scope and verification notes.
