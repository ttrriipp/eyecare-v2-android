...
style reference: for idea of the style of the design see the images named prototype in the docs folder

### Color Palette

Derived from the "Clinical Blue & White" aesthetic established in the web prototype, focusing on trust, clarity, and modern elegance.

| Token | Hex | Usage |
|-------|-----|-------|
| `primary` | `#4A90E2` | Buttons, active nav, links, selected states |
| `primaryContainer` | `#EBF8FF` | Light blue surface cards (hero areas, highlighted sections) |
| `onPrimary` | `#FFFFFF` | Text/icons on primary buttons |
| `surface` | `#FFFFFF` | Screen backgrounds, cards |
| `surfaceVariant` | `#F7FAFC` | Subtle background differentiation (list backgrounds, input fields) |
| `onSurface` | `#2D3748` | Primary text (headings, body) |
| `onSurfaceVariant` | `#718096` | Secondary text (labels, captions, timestamps) |
| `outline` | `#EDF2F7` | Borders, dividers, card strokes |
| `statusPending` | `#F6AD55` | Pending appointment/order chips |
| `statusConfirmed` | `#38A169` | Confirmed/completed chips |
| `statusCancelled` | `#E53E3E` | Cancelled/error chips |
| `statusInfo` | `#4A90E2` | Under review, rescheduled, informational chips |

Material 3 dynamic color: **Off.** Use the fixed palette above for consistency with the clinical brand.

### Typography

Use modern web fonts **Outfit** (for display/headings) and **DM Sans** (for body text) to provide a premium, highly-legible clinical feel. 

| Role | Style | Font | Weight | Size |
|------|-------|------|--------|------|
| Display | Screen titles | Outfit | SemiBold (600) | 24sp |
| Headline | Section headers | Outfit | Medium (500) | 18sp |
| Title | Card titles, list names | DM Sans | SemiBold (600) | 16sp |
| Body | General content | DM Sans | Regular (400) | 14sp |
| Label | Chips, buttons | DM Sans | Medium (500) | 12sp |
| Caption | Timestamps, helper | DM Sans | Regular (400) | 12sp |

Text color: Use `onSurface` for primary text, `onSurfaceVariant` for secondary/helper text.

### Component Conventions

**Cards:** White background, 16dp corner radius, soft drop shadow (`0 4px 16px rgba(0,0,0,0.04)`), and an optional 1dp border. 

**Buttons:**
- Primary: Filled, `primary` background, white text, pill shape (24dp radius).
- Secondary: Outlined, `primary` border and text.
- Extended FAB: Used for primary creation actions (e.g., "Book New Appointment"). Floats above content on the right, pill-shaped with an icon and text.

**Navigation Layout (Split Navigation):**
Instead of a standard flush bottom nav, use a split floating design:
- **Left Pill:** A wide, floating white pill (elevated 24dp from the bottom edge) containing the 4 primary tabs (Home, Catalog, Visits, Profile).
- **Right Companion Button:** A distinct, primary-colored floating squircle (rounded square) detached from the pill, serving exclusively as the Chat/Messages entry point.

**Input fields:** Outlined style, 12dp radius, `surfaceVariant` fill.

### Layout Principles

- **24dp horizontal padding** on main screen containers.
- **Card-based structure**: Almost all content blocks are enclosed in soft, elevated cards.
- **Scrolling**: Content must scroll fully behind the floating split navigation (add extra bottom padding).

### Screen-Specific Design Notes

**Home:** A rich, comprehensive dashboard bridging clinical health and retail e-commerce.
- **Header:** Greeting with user name and a standalone Notification Bell icon in the top right.
- **Vision Status Card:** A clinical status display (e.g., "Optimal", "20/20").
- **Prescription Warning (conditional):** A red alert card if the user's prescription is expiring soon.
- **Next Appointment:** Highlight card showing confirmed visit details.
- **Active Order Tracker:** A visual progress bar tracking glasses/lens production status.
- **New Arrivals Carousel:** Horizontal scrolling gallery of featured products.
- **Daily Eye Tip:** An educational module at the bottom.

**Product catalog:** 2-column grid, product image top. Filter pills at the top span clinical and retail (`All`, `Frames`, `Sunglasses`, `Contacts`, `Accessories`). AR-eligible badge on applicable frames.

**AR try-on:** Full-bleed camera. The UI shrinks to floating controls (variant selector as a horizontal chip row at the bottom, close button top-left, "Order" FAB bottom-right).

**Appointment booking:** Multi-step wizard with a step indicator. 

**Chat:** Single persistent conversation screen. The bottom split navigation disappears when Chat is opened to give the conversation full screen real estate. Contextual cards appear inline.

**Feedback:** Triggered from completed appointment/order detail screens.
