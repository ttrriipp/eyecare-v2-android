...
class ChatViewModelTest {
    ...
    @Test
    fun `downloadAttachment stores the result for the screen to hand off to another app`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        val download = AttachmentDownload(fileName = "receipt.pdf", mimeType = "application/pdf", bytes = byteArrayOf(1, 2, 3))
    ...
    @Test
    fun `downloadAttachment with SAVE_TO_GALLERY intent preserves it through completion`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        val download = AttachmentDownload(fileName = "eye-photo.jpg", mimeType = "image/jpeg", bytes = byteArrayOf(1))
        coEvery { repo.downloadAttachment(9) } returns Result.success(download)
        val vm = vm()
        ...
        try {
            ...
            val state = vm.uiState.value as ChatUiState.Success
            assertEquals(download, state.downloadedAttachment)
            assertEquals(AttachmentDownloadIntent.SAVE_TO_GALLERY, state.downloadIntent)
        } finally {
            vm.viewModelScope.cancel()
        }
    }
    ...
    @Test
    fun `consumeDownloadedAttachment clears the one-shot result`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        val download = AttachmentDownload(fileName = "receipt.pdf", mimeType = "application/pdf", bytes = byteArrayOf(1))
        coEvery { repo.downloadAttachment(9) } returns Result.success(download)
        val vm = vm()
    ...
    @Test
    fun `failed download surfaces a calm error instead of the raw exception, and remembers how to retry`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        ...
        try {
            ...
            val state = vm.uiState.value as ChatUiState.Success
            // Never the raw exception text - patients shouldn't see "Unable to resolve host".
            assertEquals("We couldn't open that attachment. Check your connection and try again.", state.downloadError)
            assertNull(state.downloadedAttachment)
            assertNull(state.downloadingAttachmentId)
            assertEquals(FailedDownload(9, AttachmentDownloadIntent.SAVE_TO_GALLERY), state.lastFailedDownload)
        } finally {
            vm.viewModelScope.cancel()
        }
    }
    ...
    @Test
    fun `retryDownload replays the failed attachment id and intent`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        coEvery { repo.downloadAttachment(9) } returns Result.failure(RuntimeException("offline"))
        val vm = vm()
    ...
    @Test
    fun `retryDownload does nothing when there is no failed download to replay`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        val vm = vm()
    ...
    @Test
    fun `failed send surfaces a calm error and remembers the body for retry`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        coEvery { repo.sendMessage("Hi there") } returns Result.failure(RuntimeException("Unable to resolve host"))
        val vm = vm()
        ...
        try {
            ...
            val state = vm.uiState.value as ChatUiState.Success
            assertEquals("Your message didn't send. Check your connection and try again.", state.sendError)
            assertEquals("Hi there", state.lastFailedMessageBody)
        } finally {
            vm.viewModelScope.cancel()
        }
    }
    ...
    @Test
    fun `retrySendMessage resends the last failed body`() = runTest {
        coEvery { repo.getConversation() } returns Result.success(fakeConversation)
        coEvery { repo.getMessages() } returns Result.success(emptyList())
        coEvery { repo.sendMessage("Hi there") } returns Result.failure(RuntimeException("offline"))
        val vm = vm()
        ...
        try {
            ...
            coEvery { repo.sendMessage("Hi there") } returns Result.success(fakeMessage.copy(id = 2, body = "Hi there"))
            vm.retrySendMessage()
            dispatcher.scheduler.runCurrent()
