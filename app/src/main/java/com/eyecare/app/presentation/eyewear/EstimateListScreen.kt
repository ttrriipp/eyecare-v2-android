...
fun EstimateListContent(
    ...
    onSelectFilter: (EstimateFilter) -> Unit,
    onRetry: () -> Unit,
    onLoadMore: () -> Unit,
    ...
    val currentFilter = when (uiState) {
        is EstimateListUiState.Success -> uiState.filter
        is EstimateListUiState.Empty -> uiState.filter
        else -> EstimateFilter.CURRENT
    }

    Column(modifier.fillMaxSize()) {
        EstimateFilterRow(currentFilter, onSelectFilter)
        ...
        when (val state = uiState) {
            ...
            is EstimateListUiState.Empty -> Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                Column(horizontalAlignment = Alignment.CenterHorizontally) {
                    ...
                    Text(
                        if (state.filter == EstimateFilter.CURRENT) "No current estimates"
                        else "No estimate history yet",
            ...
            is EstimateListUiState.Success -> LazyColumn(
...
@Composable
private fun EstimateFilterRow(
    currentFilter: EstimateFilter,
    onSelectFilter: (EstimateFilter) -> Unit,
) {
    Row(
        ...
        EstimateFilter.entries.forEach { filter ->
